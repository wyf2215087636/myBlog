...
public interface PostsMapper {
	...
	@Update("update admin set adminpass=#{adminpass} where adminname=#{adminname}")
	public int updateAdmins(Admin admin);
	
	@Update("update admin set sign_in=#{sign_in} where adminname=#{adminname}")
	public int updateAdmin_sign_in(Admin admin);
	
	@Select("select * from posts where title like '%${title}%'")
	public List<Posts> getWithTitle(Posts posts);
